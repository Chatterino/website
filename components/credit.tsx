...
function GitHubUser({ username }: { username: string }) {
    ...
    return (
        <a href={`https://github.com/${username}`} className="gh-author"><img src={avatarURL} className="gh-avatar" />@{username}</a>
    )
}
...
function formatUsers(users: string[] | undefined): React.ReactNode[] {
    ...
    return users.map(username => <GitHubUser username={username.trim()} />) || [<p>TODO REMOVE THIS SINCE AUTHORS SHOULD ALWAYS BE SET</p>];
}
...
export default function Credit({
    ...
    return <div className="text-gray-300 mt-3 mb-4 text-sm">
