...
### Shared moderator command improvements
...
### Should have their own things, but at the bottom since they're minor features
...
Added a separate highlight option for watchstreak notifications.
...
You can now control the character limit of deleted messages with the "Limit
length of deleted messages" setting.
...
You can now right-click any tab and sort your tabs by name alphabetically.
This is an action, not a setting meaning if you want to sort your tabs again,
	 you have to click the sort action again.
...
Similar to in browsers, you can now close multiple tabs at the same time (e.g. close other tabs, close tabs to the right etc).
...
You can now customize the /live messages to include the stream title.
...
You can now pre-define the title & duration of a clip.
Examples:
 - `/clip the title`
 - `/clip --duration 10 the title`
...
You can now create a filter based on the author's external badges.
Examples:
 - `author.external_badges contains "chatterino:Top Donator"`
 - `author.external_badges contains "frankerfacez:bot"`
