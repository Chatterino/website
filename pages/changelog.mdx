...
![img](https://i.pajlada.se/rnLdM.png)
...
### Added overlay window support

This is a transparent window you can put on top of other things - might be useful for recording things or other things.

TODO: add nicer description

TODO: add pictures

Authored by @nerixyz with help from @pajlada

#### Improved support for Twitch features

This adds headers above messages using the **Message Effects** or **Gigantify an Emote** feature. We don't actually show any message effects, nor do we gigantify the emote, but you can pretend the emote is bigger now that you know it's meant to be gigantified.

TODO: add picture

We better indicate whether messages came from the **Shared chat** feature
...
![streamer mode enabled](https://i.pajlada.se/0gXTm.png)

![streamer mode disabled](https://i.pajlada.se/Bqt1q.png)
...
#### Improved tab unread indicators
...
#### Added support for adding icons to inline moderation actions
...
Added the ability to duplicate tabs. This saves filters.

TODO: improve description & maybe a little gif

Authored by @KleberPF with help from @kornes @nerixyz @8thony @pajlada

#### Improved error handling for Twitch API requests

When using the `/settitle` and `/setgame` commands you're now told what's actually wrong.

Authored by @JakeRYW

#### Updated browser list for "Open links in private mode" feature

Added Brave, google-chrome-stable, firefox-devedition, firefox-developer-edition, firefox-beta, and firefox-nightly
...
#### Added multi-channel support for ban/timeout/untimeout/unban commands
...
#### Helix API emote loading

We now use Twitch's Helix API to load the list of emote's you can post. This means we can now see most emotes you have access to, including event emotes.

Authored by @nerixyz with help from @Felanbird @occluder @iProdigy @pajlada

#### Removed beta IRC support
...
#### Modified tab visibility control
