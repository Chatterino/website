...
- The "Created" and "Following since" dates now have a tooltip that shows the duration since since that event.
...
- In addition to the existing options like opening a split, you can now open the channel in a custom player when clicking on a twitch.tv link in chat.
...
- Through `c2.WebSocket`, plugins can now open WebSocket connections. This can be used to listen to events from thrid party services, for example.
  <Credit
    author="Nerixyz"
...
- For plugin developers, there is a new REPL (read-eval-print-loop) window similar to the browser console.
  ...
  You have to manually enable this in your [`settings.json`](https://wiki.chatterino.com/Settings/) where you have to set `/plugins/repl/enabled` to `true`.
...
- There's a new hotkey action for opening the account selector.
