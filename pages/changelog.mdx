...
overlay window of your selected split.
...
More actual links are now parsed as links, and non-links-but-things-that-look-like-links no longer parse as links.
...
- Improved subscription & resubscriptions messages when they are multi-month subs. - Authored by @iProdigy
- Chatlogs can now be organized by stream IDs rather than the current date. This makes it easier to log streams that consistently cross date boundaries. - Authored by @pajlada
...
- Fixes an issue when double-click selecting outside of a message, multiline-rendering that start at a trailing space, and some RTL text quirks. - Authored by @nerixyz
- Emote names are no longer duplicated when using smarter emote completion method. - Authored by @ilya-zlobintsev
- Shutting down Chatterino2 is now done in a more clean manner, meaning there's less chance for file corruption due to an unclean exit. - Authored by @nerixyz and @pajlada
...
- Fixed tab move animation occasionally failing to start after closing a tab. - Authored by @kornes with help from @nerixyz
...
- Fixed incorrect message being disabled in some cases upon approving or denying an automod caught message. - Authored by @iProdigy with help from @jupjohn
